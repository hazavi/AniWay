  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429 && retries > 0) {
        ...
        setTimeout(() => fetchTopAnimes(page, param, value, retries - 1), 3000);
        return;
      }
    ...
    container.innerHTML = ""; // Remove loading animation
...
function displayTopAnimes(result) {
  ...
  result.data.forEach((anime) => {
    ...
    image.src =
      anime.images.jpg.large_image_url || "path/to/placeholder-image.jpg";
    ...
    popup.innerHTML = `
            ...
            <p><strong>${
              anime.aired?.prop?.from?.year || "Year not available"
            }</strong></p>
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const activeFilterElement = document.querySelector(
    `.sub-nav a[data-param="${currentParam}"][data-value="${currentValue}"]`
  );
